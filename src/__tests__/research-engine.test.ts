...
import type { ProgressEvent } from "../contracts/schemas.js";
...
describe("ResearchEngine Seam", () => {
  ...
  it("executes complete research workflow end-to-end and returns structured results", async () => {
    ...
    assert.equal(
      ...
      "AI in healthcare is transforming medicine."
    );
  ...
  it("handles subtopic query failure gracefully without failing the entire research", async () => {
    ...
    assert.ok(
      result.researchResults["Diagnostic Imaging"].content.includes(
        "failed for this subtopic"
      )
    );
    assert.equal(
      result.researchResults["Drug Discovery"].content,
      "Healthy content"
    );
  });
