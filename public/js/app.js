...
class SmartResearchApp {
  ...
  bindEventListeners() {
    // Navigation buttons
    document
      .getElementById("startResearchBtn")
    ...
    // Form submission
    document
      .getElementById("researchFormElement")
    ...
    try {
      ...
      if (this.isConnected) {
        ...
        this.showNotification(
          "Successfully connected to research backend",
          "success"
        );
      } else {
        this.updateConnectionStatus(false, "Ollama not available");
        this.showNotification(
          "Backend connected but Ollama not available. Some features may be limited.",
          "warning"
        );
      }
    } catch (error) {
      ...
      this.showNotification(
        "Backend connection failed. Running in demo mode.",
        "warning"
      );
    }
  }
  ...
  formatResearchFindings(researchResults) {
    return Object.entries(researchResults)
      .map(
        ([title, data]) => `
            <h4>${title}</h4>
            <p>${data.content}</p>
  ...
  async animateSubsteps(stepId, substeps) {
    const substepsContainer = document.getElementById(
      stepId.replace("stepDetailed", "substeps")
    );
    if (!substepsContainer) return;

    // Update substeps content
    substepsContainer.innerHTML = substeps
      .map((step) => `<div class="substep">${step}</div>`)
      .join("");
  ...
  async generateMockResults(formData) {
    ...
    const mockResults = {
      ...
      synthesis: formData.includeSynthesis
        ? this.generateSynthesis(formData.topic)
        : null,
      followupQuestions: formData.includeFollowups
        ? this.generateFollowupQuestions(formData.topic)
        : null,
      performanceStats: {
        totalTime: "2m 34s",
        model: "qwen3:1.7b",
...
Organizations and individuals engaging with this field should focus on building adaptive capabilities, fostering innovation cultures, and maintaining awareness of evolving best practices and regulatory requirements.`;
    ...
    const baseSubtopics = [
      {
        ...
        findings:
          "The current landscape shows rapid development with increasing adoption across various sectors. Key players are establishing market positions while emerging technologies continue to reshape possibilities.",
      },
      {
        title: "Key Technologies & Methods",
        description: "Core technologies and methodological approaches",
        findings:
          "Several breakthrough technologies are converging to enable new capabilities. Methodological advances are improving efficiency and effectiveness while reducing implementation barriers.",
      },
      {
        title: "Applications & Use Cases",
        description: "Practical applications and real-world implementations",
        findings:
          "Diverse applications are emerging across industries, with early adopters demonstrating significant benefits. Use cases range from operational improvements to entirely new business models.",
      },
      {
        title: "Challenges & Considerations",
        description: "Key obstacles and important factors to consider",
        findings:
          "Implementation challenges include technical complexity, resource requirements, and regulatory considerations. Success factors involve strategic planning, stakeholder engagement, and adaptive management approaches.",
      },
      {
        title: "Future Outlook & Trends",
        description: "Emerging trends and future developments",
        findings:
          "Future developments point toward increased sophistication and broader adoption. Emerging trends suggest continued innovation with focus on sustainability, accessibility, and practical value creation.",
      },
    ];
  ...
  displayResults(results) {
    ...
    executiveSummaryEl.innerHTML = this.formatContentAsMarkdown(
    ...
    topicAnalysisEl.innerHTML = this.formatContentAsMarkdown(
    ...
    researchFindingsEl.innerHTML = this.formatContentAsMarkdown(
